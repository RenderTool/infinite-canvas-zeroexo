...
import { modelOptionLabel, decodeChannelModel } from '@/features/ai-config/use-ai-config-store.js';
...
export function useAiGeneration({
  ...
  useEffect(() => {
    ...
    // ==== AI 生成分镜(真实剧本)的复用逻辑:按集生成,供初次生成/重试/切换集共用 ====
    const runAiStoryboard = (_store: any, q: any, scriptNode: any, storyboardId: string, episodeId?: string) => {
      ...
      const decodedModel = decodeChannelModel(selectedModel ?? '');
      const effectiveModel = decodedModel?.model ?? selectedModel ?? 'gpt-4o';
      const effectiveProviderId = selectedChannelId || decodedModel?.channelId || undefined;
      const ctl = new AbortController();
      ...
      let lastProgress = 0;
      console.log('[runAiStoryboard] channel config:', { selectedChannelId, selectedModel, decodedModel, effectiveProviderId, effectiveModel });
    ...
    // ==== 多集分镜生成:串行处理每个选集,结果统一写入同一分镜节点的 shotsByEpisode ====
    const runAiStoryboardMultiEpisode = (
      _store: any, q: any, scriptNode: any, storyboardId: string, episodeIds: string[],
    ) => {
      const provider = refs.aiProvider;
      const scriptData = (scriptNode.data ?? {}) as { episodes?: Array<{ id: string; content?: string }> };
      const episodes = scriptData.episodes ?? [];
      if (!provider) {
        q.execute(new UpdateNodeDataCommand(storyboardId, { status: 'error' }));
        return;
      ...
      const allShotsByEpisode: Record<string, Shot[]> = {};
      ...
      const generateAll = async () => {
        ...
        for (const epId of episodeIds) {
          ...
          const scriptText = htmlToPlainText(targetEp?.content ?? '');
          if (!targetEp || !scriptText) {
            allShotsByEpisode[epId] = [];
            continue;
          }

          nodeCtlMap.set(epId, ctl);
          try {
            const shots = await generateStoryboardShots(
              provider, scriptText, ctl.signal, undefined, effectiveProviderId, effectiveModel,
            );
            allShotsByEpisode[epId] = shots;
          } catch (err: any) {
            if (err?.name === 'AbortError') {
              break;
            }
            ...
            nodeCtlMap.delete(epId);
          }
        }

        // 无论是否中断,都更新节点数据 + 展开到默认尺寸
        q.execute(new UpdateNodeDataCommand(storyboardId, {
          shotsByEpisode: allShotsByEpisode,
          activeEpisodeId: episodeIds[0]!,
          status: 'ready',
          isSample: false,
          sourceScriptId: scriptNode.id,
          episodeIds,
        } as Record<string, unknown>));
        q.execute(new ResizeNodeCommand(storyboardId,
          { x: nodePos.x, y: nodePos.y, width: compactSize.width, height: compactSize.height },
          { x: nodePos.x, y: nodePos.y, width: defaultSize.width, height: defaultSize.height },
        ));
        // 生成成功后自动按层排列
        triggerAutoLayoutAndFocusRef.current([storyboardId]);
      };
    ...
    // 剧本 → 生成分镜:直接创建分镜节点(紧凑占位尺寸),生成成功后再展开
    ...
    const unsubGenStory = nodeActionBus.on('script:generateStoryboard', (event: { nodeId: string; mode?: 'new' | 'reuse' | 'template' | 'ai' }) => {
      ...
      q.execute(new AddNodeCommand({
        ...
        size: compactSize,
        title,
        data: { status: 'generating', isSample: false, sourceScriptId: scriptNode.id },
      }));
      q.execute(new AddEdgeCommand({
        id: `edge-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
        source: { nodeId: scriptNode.id, pinId: 'output' },
        target: { nodeId: storyboardId, pinId: 'input' },
      }));
      store.setSelection({ selectedNodeIds: new Set([storyboardId]), selectedEdgeIds: new Set() });
      ...
      // 真实剧本 → AI 生成
      runAiStoryboard(store, q, scriptNode, storyboardId);
      // 生成中即参与自动排列，使用默认尺寸确保用户能找到节点
      triggerAutoLayoutAndFocusRef.current([storyboardId]);
    });
    ...
    const unsubAssociate = nodeActionBus.on('storyboard:associate', (event: any) => {
      ...
      if (event.targetNodeId) {
        ...
        const doAssociate = () => {
          ...
          if (event.autoGenerate && epIds.length > 0) {
            ...
            runAiStoryboard(store, q, scriptNode, tgtNodeId, first);
            for (let i = 1; i < epIds.length; i++) {
              runAiStoryboard(store, q, scriptNode, tgtNodeId, epIds[i]!);
            }
            // 生成中即参与自动排列，使用默认尺寸确保用户能找到节点
            triggerAutoLayoutAndFocusRef.current([tgtNodeId]);
          }
        ...
        if (hasExistingShots && switchingScript && event.autoGenerate) {
          modal.confirm({
            title: i18n.t('editor.linkNewScript'),
            content: i18n.t('editor.overwriteConfirmContent'),
            okText: i18n.t('editor.overwriteAndLink'),
      ...
      // 新建分镜节点(剧本侧批量):统一创建1个分镜节点(紧凑占位尺寸),多集通过内部集数下拉切换管理
      const sameTypeCount = graph.nodes.filter((n: any) => n.type === 'storyboard').length;
      const title = `${i18n.t('canvasNodes.stage.storyboard')}${sameTypeCount + 1}`;
      const storyboardId = `storyboard-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      const compactSize = { ...CREATION_COMPACT_SIZE };
      ...
      q.execute(new AddNodeCommand({
        ...
        size: compactSize,
        title,
        data: { status: 'generating', sourceScriptId: event.scriptNodeId, episodeIds: epIds, shotsByEpisode: {}, activeEpisodeId: epIds[0]! },
      }));
      q.execute(new AddEdgeCommand({
        id: `edge-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
        source: { nodeId: event.scriptNodeId, pinId: 'output' },
      ...
      if (event.autoGenerate) {
        // 多集生成:串行处理每个选集,结果统一写入同一节点的 shotsByEpisode
        runAiStoryboardMultiEpisode(store, q, scriptNode, storyboardId, epIds);
        // 生成中即参与自动排列，使用默认尺寸确保用户能找到节点
        triggerAutoLayoutAndFocusRef.current([storyboardId]);
      } else {
        ...
        q.execute(new UpdateNodeDataCommand(storyboardId, { status: 'idle' }));
        triggerAutoLayoutAndFocusRef.current([storyboardId]);
      }
    ...
    const unsubRetryStory = nodeActionBus.on('storyboard:retryGenerate', (event: { nodeId: string }) => {
      ...
      runAiStoryboard(store, q, scriptNode, event.nodeId, activeEp);
    });

    // 分镜"切换集/重新生成指定集":校验剧本连线后生成指定集
    const unsubRegenEp = nodeActionBus.on('storyboard:regenerateEpisode', (event: any) => {
      ...
      runAiStoryboard(store, q, scriptNode, event.nodeId, event.episodeId);
    });
