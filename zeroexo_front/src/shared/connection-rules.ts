...
const ALL_NODE_TYPES: AddNodeType[] = [
  'text', 'image', 'video', 'audio', 'generator', 'stacked-media', 'script', 'storyboard', 'workbench', 'subject', 'production-manager',
];
...
const COMPATIBILITY_MATRIX: Record<string, AddNodeType[]> = {
  ...
  generator: ['text', 'image', 'video', 'audio', 'stacked-media', 'generator', 'script', 'storyboard', 'workbench', 'subject', 'production-manager'],
  ...
  // 剧本:可连分镜 + 统筹(剧级资产管理器, Plan#29)
  script: ['storyboard', 'production-manager'],
  // 分镜:可连工作台 + 主体/统筹(资产关联)
  storyboard: ['workbench', 'subject', 'production-manager'],
  // 工作台:终端节点,不接下游
  workbench: [],
  // 主体节点:可连入工作台（视频生成上游）
  subject: ['workbench'],
