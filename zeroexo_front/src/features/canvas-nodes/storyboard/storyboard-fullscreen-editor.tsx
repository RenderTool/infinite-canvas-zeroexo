/**
 * StoryboardFullscreenEditor - 分镜全屏编辑（Plan#33 C2，剧管同款三栏结构）
 *
 * 复用 ProductionManagerModal 同款骨架（无边线：背景分层 + 阴影）：
 * - 顶部：分镜切换器（上/下箭头 + 计数 + 新增/删除）+ 集切换 + 关闭
 * - 中栏：单镜图册/图库（一镜对多图，AlbumPanel / ItemImageCard 复用）
 * - 右1：单图详情（一图一提示词，SelectedImageDetail 复用）
 * - 右2：分镜行表单（storyboardTable 列字段表单化，实时回写无保存按钮）
 */
import { memo, useCallback, useEffect, useMemo, useRef, useState, type ReactElement } from 'react';
import { Modal, Tooltip, App as AntdApp, Select, Empty } from 'antd';
import {
  ChevronLeft, ChevronRight, Plus, Trash2, X, ListVideo, Loader2, LayoutGrid, Image as ImageIcon, AtSign,
} from 'lucide-react';
...
import type { Shot, EntityKind } from './storyboard-types';
...
export const StoryboardFullscreenEditor = memo(function StoryboardFullscreenEditor({
  ...
  // ===== 当前分镜（纯视图态 activeIndex，不落 node.data） =====
  ...
  const handleAdd = useCallback(() => { const before = shots.length; onAddShot(); setActiveIndex(before); }, [shots.length, onAddShot]);
