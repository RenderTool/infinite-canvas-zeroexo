  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%', minHeight: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        {/* 信息条（剧管同款：编号 + 景别徽章 + 描述摘要） */}
        <div style={{
          flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: infoBg,
        }}>
          {activeShot && (
            <>
              ...
              {dialogue && (
                <span style={{ flexShrink: 1, minWidth: 0, fontSize: 10, color: textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '38%', opacity: 0.85 }}>
                  “{dialogue}”
                </span>
              )}
              {!readOnly && promptText && (
