/**
 * subject-editor-styles - 主体编辑器样式
 *
 * 复用原则（用户强约束，禁止自由发挥）：
 * - 输入框/表单/图片查看器 = 与 asset-library/prompt-create-page.tsx **1:1 同款**
 *   （逐行提取，来源行号见注释；改动须同步修改提示词页面保持一致）
 * - 网格卡片 = 与 asset-library/cards/asset-card.tsx **1:1 同款**
 *   （封面 239.2/135.4 + cover 填充 + 底部两行信息 + hover 浮条）
 * - 块间分隔：背景分层 + 阴影，慎用边线；控件（输入框/按钮）按提示词页面保留边线
 */
